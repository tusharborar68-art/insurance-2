...
st.markdown("""
This interactive dashboard assists in audit processes to detect systemic biases in insurance claim approvals and rejections, 
backed by descriptive metrics, targeted diagnostic graphs, and stable predictive modeling.
""")
...
if df is not None:
    ...
    with tab1:
        ...
        ct = pd.crosstab(df[selected_cat], df['POLICY_STATUS'])
        ct_norm = pd.crosstab(df[selected_cat], df['POLICY_STATUS'], normalize='index') * 100
        ct_summary = pd.concat([ct, ct_norm.rename(columns=lambda x: str(x) + ' (%)')], axis=1)
        st.dataframe(ct_summary)

        fig, ax = plt.subplots(figsize=(10, 4.5))
        ct_norm.plot(kind='bar', stacked=True, color=['#2ec4b6', '#e71d36'], ax=ax)
    ...
    with tab2:
        ...
        if diag_type == "Income-wise Bias":
            ...
            df_bins = df.copy()
            df_bins['INCOME_BIN'] = pd.cut(df_bins['PI_ANNUAL_INCOME'], bins=[-1, 1, 100000, 300000, 500000, 100000000], 
                                           labels=['No Income Marked', 'Low-Income Range (under 100k)', 'Medium Income (100k-300k)', 'High Income (300k-500k)', 'Elite Range (over 500k)'])
            st.write("**Repudiation Probabilities Categorized by Income Tiers:**")
            st.dataframe(pd.crosstab(df_bins['INCOME_BIN'], df_bins['POLICY_STATUS'], normalize='index') * 100)
            ...
            zone_summary = pd.DataFrame({
                ...
                'Rejection Rate (%)': zone_ct_norm['Repudiate Death'] if 'Repudiate Death' in zone_ct_norm.columns else 0
    ...
    with tab3:
        ...
        st.dataframe(df_metrics.style.highlight_max(axis=0, color='#e2ece9').format(precision=4))
    ...
    with tab5:
        ...
        st.markdown("""
        ### 🚨 Summary of Discovered Biases
        The initial operational assumptions regarding bias inside the settlement office are **empirically supported** by statistical evidence:
        
        1. **Socioeconomic Discriminatory Variance:** Lower income tiers (under 100k) feature a severe **51.52% rejection rate**, while upper income bands (over 500k) enjoy smooth approvals with only a **21.79% rejection rate**.
        2. **Regional Operations Variance:** Claims evaluated in **JKB JAMMU** and the **South** regional branches show disproportionate rejection frequencies exceeding **54%**, indicating unstandardized claim review protocols.
        3. **Administrative Constraints:** Policy configurations utilizing high-frequency premium channels like **Quarterly payments** suffer from elevated repudiation velocities (55.00% rejections) compared to single premium products (10.14% rejections).
        
        ### 📋 Compliance Recommendations
        * **SOP Standardization:** Mandate uniform claim investigation checklists for outliers like the **South** and **JKB JAMMU** branches.
        * **Vulnerability Framework:** Establish an administrative threshold review for lower-income segments to ensure rejections are driven by verifiable policy breaches rather than documentation resource discrepancies.
        """)